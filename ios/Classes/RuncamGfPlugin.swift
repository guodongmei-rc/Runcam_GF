...
public class RuncamGfPlugin: NSObject, FlutterPlugin {
    ...
    /// 文件选择器通道。强引用持有(它是 channel 的 handler + UIDocumentPicker delegate)。
...
/// 编辑器用的「文档选择器」通道,对齐安卓 ACTION_OPEN_DOCUMENT 与老原生页的 UIDocumentPicker。
/// asCopy:.import → 返回原始文件在 app 沙盒里的副本路径(陀螺/GPMF 轨道完整,且无需 security-scope),
/// 直接喂给 load_video_file。不走相册(image_picker),避免 Photos 导出转码剥掉陀螺遥测。
final class VideoPickerChannel: NSObject, UIDocumentPickerDelegate {
  ...
  private static let folderBookmarksKey = "gf_granted_folder_bookmarks"
